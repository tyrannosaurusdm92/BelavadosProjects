...
Use the flattened `Belavados_Effect_Studio_GitHub_Deploy_Ready.zip` package for GitHub Pages. It removes legacy v1 source HTML from the deploy package, adds cache-busting query strings, and uses a network-first service worker for HTML so GitHub cannot stay stuck on the older v1 shell after an update.
