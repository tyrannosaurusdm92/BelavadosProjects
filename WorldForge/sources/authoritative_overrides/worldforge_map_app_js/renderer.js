...
void main(){
  vWorld=geoPosition(aGeo); vColor=aColor; vElevation=aGeo.z;
  gl_Position=uViewProj*vec4(vWorld,1.0);
}`;
const TERRAIN_FS = `#version 300 es
precision highp float;
in vec3 vColor; in vec3 vWorld; in float vElevation;
...
void main(){
  ...
  vec3 base=mix(vColor,vec3(.9,.96,.98),snow*.78);
...
export class WorldRenderer {
  constructor(canvas){
    ...
    this.world={vao:null,count:0}; this.lines={vao:null,count:0}; this.local={vao:null,count:0}; this.structures={vao:null,count:0}; this.life={vao:null,count:0,positions:null,velocities:null,posBuffer:null}; this.localObjectMesh={vao:null,count:0}; this.localObjects=[]; this.points={vao:null,count:0};
  ...
  updateFeaturePoints(){
    if(!this.model)return;const pos=new Float32Array(this.features.length*3);
    for(let i=0;i<this.features.length;i++){const p=this.featurePosition(this.features[i]);pos.set(p,i*3);}
    if(this.points?.count===this.features.length&&this.points.posBuffer){
    ...
    const colors=[],sizes=[];for(const f of this.features){colors.push(...colorForType(f.type));sizes.push(f.type?.toLowerCase().includes('volcano')?15:10);}
  ...
  drawWorld(vp){
    ...
    let p=this.programs.terrain;gl.useProgram(p);this.setCommonWorldUniforms(p,vp);gl.uniform1f(this.uniform(p,'uTime'),this.time);gl.bindVertexArray(this.world.vao);gl.drawElements(gl.TRIANGLES,this.world.count,gl.UNSIGNED_INT,0);
  ...
  drawLocal(vp){
    ...
    if(this.flags.life&&this.localObjectMesh?.count){gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE);this.drawPoints(this.localObjectMesh,vp);gl.disable(gl.BLEND);}
