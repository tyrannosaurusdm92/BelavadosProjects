...
function loadSheet(obj){
  ...
  updateCalculated(); refreshArtSlots();  if(window.refreshBelavadosDropdownBuilder) window.refreshBelavadosDropdownBuilder({fromLoad:true});
}
...
function clearSheet(){
  ...
  updateCalculated(); refreshArtSlots();  if(window.refreshBelavadosDropdownBuilder) window.refreshBelavadosDropdownBuilder({fromClear:true});
...
function refreshMediaPlaceholder(){ return; }
...
updateCalculated(); refreshArtSlots();  setStatus(`Loaded ${APP_OPTIONS.fieldCount} extracted data-key fields into long-scroll modules.`);
